...
export class RecruitmentService {
  ...
  async createJobTemplate(dto: any, currentUserId: string) {
    ...
    if (!dto.department || typeof dto.department !== 'string' || dto.department.trim().length === 0) {
  ...
  async getAllApplications(
    ...
    if (prioritizeReferrals) {
      ...
      for (const app of applications) {
        const candidateId = (app as any).candidateId?._id?.toString() || (app as any).candidateId?.toString();
  ...
  async updateApplicationStatus(
    ...
    if (oldStatus === ApplicationStatus.REJECTED && dto.status !== ApplicationStatus.REJECTED) {
      throw new BadRequestException('Cannot change status of a rejected application. Rejected applications cannot be reactivated.');
    }
    if (oldStatus === ApplicationStatus.HIRED && dto.status !== ApplicationStatus.HIRED) {
      throw new BadRequestException('Cannot change status of a hired application. Hired applications are final.');
    }
    ...
    if (oldIndex > -1 && newIndex > -1 && newIndex < oldIndex && dto.status !== ApplicationStatus.REJECTED) {
      throw new BadRequestException(
      ...
      if (reqId) {
        ...
        if (dto.status === ApplicationStatus.HIRED) {
          ...
          if (requisition && hiredCount >= requisition.openings) {
            await this.jobModel.findByIdAndUpdate(reqId, { publishStatus: 'closed' });
  ...
  async createOffer(dto: CreateOfferDto, currentUserId: string) {
    ...
    const existingOffer = await this.offerModel.findOne({ applicationId: new Types.ObjectId(dto.applicationId) });
    if (existingOffer) {
    ...
    if (dto.signingBonus !== undefined && (dto.signingBonus < 0 || !Number.isFinite(dto.signingBonus))) {
  ...
  async finalizeOffer(
    ...
    if (offer.applicantResponse === OfferResponseStatus.PENDING) {
      throw new BadRequestException('Cannot finalize offer: Candidate has not responded yet. Please wait for candidate response.');
    }
    ...
    if (dto.finalStatus === OfferFinalStatus.APPROVED && offer.applicantResponse === OfferResponseStatus.ACCEPTED) {
      try {
        ...
        if (application && application._id) {
          ...
          if (application.requisitionId) {
            ...
            await this.jobModel.findByIdAndUpdate(application.requisitionId, { progress });
          }
...
Due: ${context.dueDate}` : '';
  ...
  async getOnboardingByEmployeeId(employeeId: string, _currentUserId: string): Promise<any> {
    try {
      ...
      const completedTasks = onboarding.tasks?.filter((task: any) => 
        task.status === OnboardingTaskStatus.COMPLETED
      ).length || 0;
      const inProgressTasks = onboarding.tasks?.filter((task: any) => 
        task.status === OnboardingTaskStatus.IN_PROGRESS
      ).length || 0;
      const pendingTasks = onboarding.tasks?.filter((task: any) => 
        task.status === OnboardingTaskStatus.PENDING
      ).length || 0;
      const overdueTasks = onboarding.tasks?.filter((task: any) => {
        if (!task.deadline) return false;
        return new Date(task.deadline) < new Date() && task.status !== OnboardingTaskStatus.COMPLETED;
      }).length || 0;

      const progressPercentage = totalTasks > 0 ? Math.round((completedTasks / totalTasks) * 100) : 0;

      // Get next task to complete
      const nextTask = onboarding.tasks?.find((task: any) => 
        task.status === OnboardingTaskStatus.PENDING
      );
  ...
  async updateOnboardingTask(
    ...
    try {
      ...
      if (updateTaskDto.status && !Object.values(OnboardingTaskStatus).includes(updateTaskDto.status)) {
      ...
      } else if (updateTaskDto.status && (onboarding.tasks[taskIndex] as any).completedAt) {
      ...
      const allCompleted = onboarding.tasks.length > 0 && onboarding.tasks.every((task) => (task as any).status === OnboardingTaskStatus.COMPLETED);
      if (allCompleted) {
  ...
  async addTaskToOnboarding(
    ...
    try {
      ...
      if (!taskDto.department || typeof taskDto.department !== 'string' || taskDto.department.trim().length === 0) {
  ...
  async createEmployeeFromContract(
    ...
    try {
      ...
      const offer = await this.offerModel.findById(offerId).lean() as (Offer & { _id: Types.ObjectId }) | null;
      if (!offer) {
        throw new NotFoundException('Offer not found');
      }
      ...
      if (!workEmail) {
        const firstName = candidate.firstName?.toLowerCase().replace(/\s+/g, '').replace(/[^a-z0-9]/g, '') || 'user';
        const lastName = candidate.lastName?.toLowerCase().replace(/\s+/g, '').replace(/[^a-z0-9]/g, '') || '';
      ...
      throw new BadRequestException('Failed to create employee from contract: ' + error.message);
    }
  }
  ...
  async tagCandidateAsReferral(
    ...
    try {
      ...
      throw new BadRequestException('Failed to tag candidate as referral: ' + error.message);
    }
  }
  ...
  async recordCandidateConsent(
    ...
    try {
      ...
      throw new BadRequestException('Failed to record candidate consent: ' + error.message);
    }
  }
  ...
  async submitInterviewFeedback(
    ...
    try {
      ...
      if (panelIds.length === 0) {
        throw new BadRequestException('Interview panel is empty. Cannot submit feedback without panel members.');
      }
      ...
      throw new BadRequestException('Failed to submit interview feedback: ' + error.message);
    }
  }
  ...
  async getInterviewAverageScore(
    ...
    const feedbacks = await this.assessmentResultModel.find({
      interviewId: new Types.ObjectId(interviewId),
    }).lean();
  ...
  async sendOnboardingReminders(_currentUserId: string): Promise<void> {
    try {
      ...
      for (const onboarding of allOnboardings) {
        ...
        const overdueTasks = onboarding.tasks?.filter((task: any) => {
          if (!task.deadline || task.status === OnboardingTaskStatus.COMPLETED) return false;
  ...
  async provisionSystemAccess(
    ...
    try {
      ...
      console.log(`Provisioning system access for employee ${employeeId}: ${(task as any).name}`);
      ...
      throw new BadRequestException('Failed to provision system access: ' + error.message);
    }
  }
  ...
  async reserveEquipment(
    ...
    try {
      ...
      throw new BadRequestException('Failed to reserve equipment: ' + error.message);
    }
  }
  ...
  async scheduleAccessProvisioning(
    ...
    try {
      ...
      throw new BadRequestException('Failed to schedule access provisioning: ' + error.message);
    }
  }
  ...
  async triggerPayrollInitiation(
    ...
    try {
      ...
      const payrollTask = onboarding.tasks.find((task: any) => 
        task.department === 'HR' && (task.name.includes('Payroll') || task.name.includes('payroll'))
      );
      ...
      (payrollTask as any).notes = ((payrollTask as any).notes || '') + 
      ...
      throw new BadRequestException('Failed to trigger payroll initiation: ' + error.message);
    }
  }
  ...
  async processSigningBonus(
    ...
    try {
      ...
      const bonusTask = onboarding.tasks.find((task: any) => 
        task.department === 'HR' && (task.name.includes('Signing Bonus') || task.name.includes('signing bonus'))
      );
      ...
      (bonusTask as any).notes = ((bonusTask as any).notes || '') + 
      ...
      throw new BadRequestException('Failed to process signing bonus: ' + error.message);
    }
  }
  ...
  async cancelOnboarding(
    ...
    try {
      ...
      if (!reason || typeof reason !== 'string' || reason.trim().length === 0) {
        throw new BadRequestException('Cancellation reason is required and must be a non-empty string');
      }
      ...
      for (const task of onboarding.tasks) {
        ...
        if (taskStatus === OnboardingTaskStatus.PENDING || taskStatus === OnboardingTaskStatus.IN_PROGRESS) {
      ...
      if (onboarding.tasks.length > 0) {
        (onboarding.tasks[0] as any).notes = ((onboarding.tasks[0] as any).notes || '') + 
      ...
      throw new BadRequestException('Failed to cancel onboarding: ' + error.message);
    }
  }
