...
export class PolicyConfigService {
  constructor(
    ...
    @InjectModel(LatenessRule.name) private latenessRuleModel: Model<LatenessRule>,
  ...
  async createOvertimeRule(createOvertimeRuleDto: CreateOvertimeRuleDto) {
    const newOvertimeRule = new this.overtimeRuleModel(createOvertimeRuleDto);
    return newOvertimeRule.save();
  }

  // Get all overtime rules with optional filters
  async getOvertimeRules(getPoliciesDto: GetPoliciesDto) {
    const query: any = {};
  ...
  async updateOvertimeRule(id: string, updateOvertimeRuleDto: UpdateOvertimeRuleDto) {
    return this.overtimeRuleModel.findByIdAndUpdate(id, updateOvertimeRuleDto, { new: true }).exec();
  }
  ...
  async createLatenessRule(createLatenessRuleDto: CreateLatenessRuleDto) {
    const newLatenessRule = new this.latenessRuleModel(createLatenessRuleDto);
    return newLatenessRule.save();
  }

  // Get all lateness rules with optional filters
  async getLatenessRules(getPoliciesDto: GetPoliciesDto) {
    const query: any = {};
  ...
  async updateLatenessRule(id: string, updateLatenessRuleDto: UpdateLatenessRuleDto) {
    return this.latenessRuleModel.findByIdAndUpdate(id, updateLatenessRuleDto, { new: true }).exec();
  }
  ...
  async getHolidays(getHolidaysDto: GetHolidaysDto) {
    const query: any = {};
    ...
    if (getHolidaysDto.startDate && getHolidaysDto.endDate) {
      query.$or = [
        {
          startDate: { $gte: getHolidaysDto.startDate, $lte: getHolidaysDto.endDate },
        },
        {
          endDate: { $gte: getHolidaysDto.startDate, $lte: getHolidaysDto.endDate },
        },
        {
          $and: [
  ...
  async checkHoliday(checkHolidayDto: CheckHolidayDto) {
  ...
  async validateAttendanceHoliday(validateAttendanceHolidayDto: ValidateAttendanceHolidayDto) {
    const { employeeId, date, suppressPenalty } = validateAttendanceHolidayDto;
    
    const holidayCheck = await this.checkHoliday({ date });
