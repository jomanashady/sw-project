...
export class NotificationService {
  ...
  async getNotificationLogsByEmployee(getNotificationLogsByEmployeeDto: GetNotificationLogsByEmployeeDto) {
    return this.notificationLogModel.find({ to: getNotificationLogsByEmployeeDto.employeeId }).exec();
  }

  // ===== PAYROLL SYNCHRONIZATION =====
  // These methods return actual data for Payroll/Leaves subsystems to consume

  async syncAttendanceWithPayroll(syncAttendanceWithPayrollDto: SyncAttendanceWithPayrollDto) {
    ...
    return {
      ...
      summary: {
        ...
        totalWorkHours: Math.round((attendance.reduce((sum, r) => sum + (r.totalWorkMinutes || 0), 0) / 60) * 100) / 100,
      },
    };
  }

  async syncLeaveWithPayroll(syncLeaveWithPayrollDto: SyncLeaveWithPayrollDto) {
  ...
  async synchronizeAttendanceAndPayroll(synchronizeAttendanceAndPayrollDto: SynchronizeAttendanceAndPayrollDto) {
  ...
  async getAttendanceDataForSync(employeeId: string, startDate?: Date, endDate?: Date) {
    const query: any = { employeeId };
  ...
  async getOvertimeDataForSync(employeeId: string, startDate?: Date, endDate?: Date) {
    const query: any = {
      employeeId,
    ...
    return {
      ...
      summary: {
        ...
        totalOvertimeHours: Math.round((overtimeData.reduce((sum, r) => sum + r.overtimeMinutes, 0) / 60) * 100) / 100,
      },
    };
  }
