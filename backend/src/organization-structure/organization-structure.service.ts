...
export class OrganizationStructureService {
  ...
  async getDepartmentById(id: string): Promise<DepartmentDocument> {
    const department = await this.departmentModel.findById(id).populate('headPositionId').exec();
    if (!department) {
  ...
  async updateDepartment(id: string, dto: UpdateDepartmentDto): Promise<DepartmentDocument> {
  ...
  async createPosition(dto: CreatePositionDto): Promise<PositionDocument> {
    ...
    try {
      ...
      if (payload.reportsToPositionId !== undefined) {
        ...
        payload.reportsToPositionId = new Types.ObjectId(payload.reportsToPositionId);
      }
  ...
  async getAllPositions(departmentId?: string, isActive?: boolean): Promise<PositionDocument[]> {
    const filter: any = {};
  ...
  async updatePosition(id: string, dto: UpdatePositionDto): Promise<PositionDocument> {
  ...
  async getPositionHierarchy(positionId: string): Promise<any> {
    ...
    return {
      ...
      subordinates: await Promise.all(
        subordinates.map((sub) => this.getPositionHierarchy(sub._id.toString()))
      ),
    };
  }
  ...
  async createPositionAssignment(
    ...
    if (overlapping) {
      throw new ConflictException('Employee already has an active assignment in this period');
    }
  ...
  async getPositionAssignments(positionId: string): Promise<PositionAssignmentDocument[]> {
    return this.assignmentModel
      .find({ positionId })
  ...
  async endPositionAssignment(id: string, endDate: Date): Promise<PositionAssignmentDocument> {
  ...
  async getChangeRequestById(id: string): Promise<StructureChangeRequestDocument> {
  ...
  async submitChangeRequest(
    ...
    request.submittedByEmployeeId = new Types.ObjectId(dto.submittedByEmployeeId);
  ...
  async cancelChangeRequest(id: string): Promise<StructureChangeRequestDocument> {
    const request = await this.changeRequestModel.findById(id);
    if (!request) {
      throw new NotFoundException(`Change request with ID ${id} not found`);
    }
  ...
  async createApproval(dto: CreateStructureApprovalDto): Promise<StructureApprovalDocument> {
  ...
  async getRequestApprovals(changeRequestId: string): Promise<StructureApprovalDocument[]> {
    return this.approvalModel.find({ changeRequestId }).populate('approverEmployeeId').exec();
  }
  ...
  private async logChange(
    ...
    await this.changeLogModel.create({
      ...
      beforeSnapshot,
      afterSnapshot,
      performedByEmployeeId: performedBy ? new Types.ObjectId(performedBy) : undefined,
    });
  }

  private async checkAndUpdateRequestStatus(changeRequestId: Types.ObjectId): Promise<void> {
    const approvals = await this.approvalModel.find({ changeRequestId });
    const allDecided = approvals.every((a) => a.decision !== ApprovalDecision.PENDING);

    if (!allDecided) return;

    const hasRejection = approvals.some((a) => a.decision === ApprovalDecision.REJECTED);
